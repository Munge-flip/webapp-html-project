document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateOrderSummary() {
        ...
        // Display selected regions (NO PRICE - just info)
        if (selectedServices.explorations.length > 0) {
            summaryHTML += `
                <div class="selected-item" style="background: #f0f0f0; padding: 10px; border-radius: 8px; margin-top: 10px;">
                    <span style="font-weight: 600;">Selected Regions:</span>
    ...
    form.addEventListener('submit', function(e) {
        ...
        alert(`Order placed successfully!\n\nServices: ${Object.keys(selectedServices).filter(k => k !== 'explorations' && selectedServices[k]).map(k => selectedServices[k].name).join(', ')}\n\nRegions: ${selectedServices.explorations.join(', ') || 'None'}\n\nTotal: ₱${totalPriceElement.textContent}\n\nRedirecting to your profile...`);
